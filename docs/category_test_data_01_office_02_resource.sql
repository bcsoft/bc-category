...
where rid in
      (select id
       ...
       where code like 'BC_OFFICE_TYPE_%');
delete from bc_identity_role_resource
where rid in
      (select id
       from BC_IDENTITY_ROLE
...
insert into BC_IDENTITY_ROLE (ID, STATUS_, INNER_, TYPE_, ORDER_, CODE, NAME)
  ...
  where not exists(select 0
                   from BC_IDENTITY_ROLE
                   where CODE = 'BC_OFFICE_TYPE_MANAGE');
...
insert into BC_IDENTITY_ROLE (ID, STATUS_, INNER_, TYPE_, ORDER_, CODE, NAME)
  ...
  where not exists(select 0
                   from BC_IDENTITY_ROLE
                   where CODE = 'BC_OFFICE_TYPE_READ');
...
insert into BC_IDENTITY_ACTOR (ID, UID_, STATUS_, INNER_, TYPE_, CODE, NAME, ORDER_, PCODE, PNAME)
  select NEXTVAL('CORE_SEQUENCE'), 'group.init.' || NEXTVAL('CORE_SEQUENCE'), 0, false, 3, 'officeManageGroup',
    '办公用品分类管理岗位', '9924', '[1]baochengzongbu', '宝城'
  from BC_DUAL
  where not exists(select 0
                   from BC_IDENTITY_ACTOR
                   where CODE = 'officeManageGroup');
...
insert into BC_IDENTITY_ACTOR (ID, UID_, STATUS_, INNER_, TYPE_, CODE, NAME, ORDER_, PCODE, PNAME)
  select NEXTVAL('CORE_SEQUENCE'), 'group.init.' || NEXTVAL('CORE_SEQUENCE'), 0, false, 3, 'officeReadGroup',
    '办公用品分类查阅岗位', '9925', '[1]baochengzongbu', '宝城'
  from BC_DUAL
  where not exists(select 0
                   from BC_IDENTITY_ACTOR
                   where CODE = 'officeReadGroup');
insert into BC_IDENTITY_ACTOR_RELATION (TYPE_, MASTER_ID, FOLLOWER_ID)
  select 0, am.id, af.id
  from BC_IDENTITY_ACTOR am, BC_IDENTITY_ACTOR af
  where am.CODE = 'baochengzongbu'
