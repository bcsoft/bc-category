...
insert into bc_template (id,order_,code,content,file_date,author_id,modifier_id,modified_date,subject,
	path,inner_,desc_,status_,version_,category,type_id,size_,formatted,uid_)
	select nextval('core_sequence'),'5006','BC-SELECT-CATEGORY',null,now(),
		(select id from bc_identity_actor_history where actor_code='admin' and current=true),
		(select id from bc_identity_actor_history where actor_code='admin' and current=true),now(),
		'选择分类','bc/categorySelectView.v1.sql.ftl',false,'选择分类视图SQL主模板',0,'1.0','平台/分类管理/选择分类',
		(select id from bc_template_type where code='freemarker'),
		0,false,'Template.category.3'
	from BC_DUAL 
	where not exists (select 0 from bc_template where code='BC-SELECT-CATEGORY');
	
-- 平台/分类管理/选择分类计数
insert into bc_template (id,order_,code,content,file_date,author_id,modifier_id,modified_date,subject,
	path,inner_,desc_,status_,version_,category,type_id,size_,formatted,uid_)
	select nextval('core_sequence'),'5007','BC-SELECT-CATEGORY-COUNT',null,now(),
		(select id from bc_identity_actor_history where actor_code='admin' and current=true),
		(select id from bc_identity_actor_history where actor_code='admin' and current=true),now(),
		'选择分类计数','bc/categorySelectView.v1.count.sql.ftl',false,'选择分类视图SQL计数模板',0,'1.0','平台/分类管理/选择分类计数',
		(select id from bc_template_type where code='freemarker'),
		0,false,'Template.category.4'
	from BC_DUAL 
	where not exists (select 0 from bc_template where code='BC-SELECT-CATEGORY-COUNT');
