...
delete from BC_IDENTITY_ACTOR_RELATION where 
	FOLLOWER_ID in (select id from BC_IDENTITY_ACTOR where name like '%物资分类分类%')
	or MASTER_ID in (select id from BC_IDENTITY_ACTOR where name like '%物资分类分类%');
delete from bc_subscribe_actor where aid in (
	select id from BC_IDENTITY_ACTOR where name like '%物资分类分类%'
);
delete from BC_IDENTITY_ACTOR where name like '%物资分类分类%';
...
insert into BC_IDENTITY_RESOURCE (ID,STATUS_,INNER_,TYPE_,BELONG,ORDER_,NAME,URL,ICONCLASS) 
	...
	and not exists (select 0 from BC_IDENTITY_RESOURCE where NAME='物资分类');
...
insert into BC_IDENTITY_ACTOR (ID,UID_,STATUS_,INNER_,TYPE_,CODE, NAME, ORDER_,PCODE,PNAME) 
	...
	, 'assetManageGroup','物资分类管理岗', '9926','[1]baochengzongbu','宝城'
	from BC_DUAL
...
insert into BC_IDENTITY_ACTOR (ID,UID_,STATUS_,INNER_,TYPE_,CODE, NAME, ORDER_,PCODE,PNAME) 
	select NEXTVAL('CORE_SEQUENCE'),'group.init.'||NEXTVAL('CORE_SEQUENCE'), 0, false, 3
	, 'assetReadGroup','物资分类查阅岗', '9927','[1]baochengzongbu','宝城'
	from BC_DUAL
